...
func TestSessionCallbackMultiple(t *testing.T) {
	// 创建一个简单的 session 对象用于测试
	s := &session{
		once: &sync.Once{},
		done: make(chan struct{}),
...
func TestInvokeCloseCallbacks(t *testing.T) {
	// 创建一个简单的 session 对象用于测试
	s := &session{
		once: &sync.Once{},
		done: make(chan struct{}),
...
func TestInvokeCloseCallbacksEmpty(t *testing.T) {
	// 测试空回调列表的情况
	s := &session{
		once: &sync.Once{},
		done: make(chan struct{}),
